...
```{r genes_detected_plot, include=TRUE, eval=FALSE}
...
rgl.postscript( filename = file.path(rootPath, 'plots', 'Figure.1e.eps'), 
                fmt = 'eps', drawText = TRUE 
)
rgl.postscript( filename = file.path(rootPath, 'plots', 'Figure.1e.notext.eps'), 
                fmt = 'eps', drawText = FALSE 
)
rgl.snapshot( filename = file.path(rootPath, 'plots', 'Figure.1e.png'), 
              fmt = "png")
par3d(op)
```
    ...
    for( secondPC in seq_len( length(PCDataList) ) ){
      ...
      if( firstPC == secondPC ){
        if(profile_type == 'heatmap'){
          ...
          plot <- profile_for_PC( firstPC, PCDataList, size, ... )
...
matrixPlotList <- PCA_matrixPlot( PCDataList, pca, ddsRlog, 
                                  tpms = tpmFilt, numGenes = 100, size = 2 )

# save as both pdf and eps
matrixFile <- file.path(rootPath, 'plots', 'Fig.S2.pdf' )
ggsave(matrixFile, width = 20, height = 20, marrangeGrob(matrixPlotList, nrow = 6, ncol = 6) )

matrixPlotList <- PCA_matrixPlot( PCDataList, pca, ddsRlog, 
                                  tpms = tpmFilt, numGenes = 100, size = 1 )
matrixFile <- file.path(rootPath, 'plots', 'Fig.S2.eps' )
ggsave(matrixFile, width = 7, height = 7, marrangeGrob(matrixPlotList, nrow = 6, ncol = 6), device = 'eps' )
...
plotList <- add_to_plot_list( plotList, 
                              ...
                              file.path(rootPath, 'plots', 'Figure.3a-zfa-heatmap.pdf') )
...
do
for stage in $( find topgo/maxByStage/ | grep "$ontology.topGO.pdf" | sed -e "s|topgo/maxByStage/||; s|.$ontology.topGO.pdf||" )
do
perl ./parse_GO_results.pl \
--enriched --sig_level $adjustedP \
--output_file topgo/maxByStage/$stage.$ontology.topGO.sig.tsv \
topgo/maxByStage/$stage.$ontology.topGO.tsv \
topgo/maxByStage/$stage.gene-list.tsv > topgo/maxByStage/$stage.$ontology.enriched-GO.tsv
done
done
```
...
pdf(file = file.path(rootPath, 'plots', 'biolayout-topgo-summary-plots.pdf'))
ggplot(data = topgo_results) + 
  geom_histogram( aes(x = cluster), stat = 'count' ) +
  labs(x = 'Cluster', y = 'Number of GO terms enriched') +
  theme( axis.text.x = element_blank(),
         axis.ticks.x = element_blank() )

topgo_summary <- ddply(topgo_results, .(cluster), summarise, 
                       number_enrichments = length(GO.ID),
                       number_annotations = sum(Annotated))

cluster_sizes <- as.integer(table(clusterData$cluster)[ topgo_summary$cluster ])
topgo_summary$size <- cluster_sizes

ggplot(data = topgo_summary) + 
  geom_point( aes(x = size, y = number_enrichments) ) +
  labs(x = 'Cluster Size', y = 'Number of GO terms enriched')

ggplot(data = topgo_summary) + 
  geom_point( aes(x = size, y = number_annotations) ) + 
  labs(x = 'Cluster Size', y = 'Number of Gene to GO term annotations')

ggplot(data = topgo_summary) + 
  geom_point( aes(x = number_annotations, y = number_enrichments, size = size) ) +
  labs(x = 'Number of Annotations to genes in cluster',
       y = 'Number of GO enrichments')

topgo_summary_by_domain <- 
  ddply(topgo_results, .(cluster, domain), summarise, 
        number_enrichments = length(GO.ID),
        number_annotations = sum(Annotated))
cluster_sizes <- as.integer(table(clusterData$cluster)[ topgo_summary_by_domain$cluster ])
topgo_summary_by_domain$size <- cluster_sizes

ggplot(data = topgo_summary_by_domain) + 
  geom_point( aes(x = number_annotations, y = number_enrichments, size = size) ) +
...
topgo_collapsed_results_file <- file.path(rootPath, 'output/topgo/0.94-e85-fisher/topGOResults.sig.collapsed.tsv')
write.table(collapsed_go_results, topgo_collapsed_results_file,
            quote = FALSE, row.names = FALSE, col.names = TRUE, sep = "\t")


# topgo_results_file <- file.path(rootPath, 'output/topgo/0.94-e85/topGOResults.sig.tsv')
# topgo_results <- read.table(topgo_results_file, sep = "\t", header = TRUE)
# collapsed_go_results <- 
#   do.call(rbind,
#           lapply(split(topgo_results, topgo_results$cluster),
...
#   )
# # output collapsed results
# topgo_collapsed_results_file <- file.path(rootPath, 'output/topgo/0.94-e85/topGOResults.sig.collapsed.tsv')
# write.table(collapsed_go_results, topgo_collapsed_results_file,
#             quote = FALSE, row.names = FALSE, col.names = TRUE, sep = "\t")
# 
```
...
for( cluster_num in seq_len(length(clusters))){
  ...
  profile_data <- melt(colMeans(t(scale(t(meanTpm_for_cluster)))))
  profile_data$stage <- factor( row.names(profile_data),
                                levels = row.names(profile_data) )
  ...
  print(ggplot(data = profile_data, aes(x = stage, y = value, group = 1) ) + 
...
# tpm_for_cluster <- tpmFilt[as.character(clusters[[254]]$gene_id), ]
# profile_data <- melt(colMeans(t(scale(t(tpm_for_cluster)))))
# profile_data$sample <- row.names(profile_data)
# profile_data$stage <- factor( sub('-[0-9]$', '', profile_data$sample),
#                               levels = unique(sub('-[0-9]$', '', profile_data$sample)) )
...
# remove "unspecified"
biolayout_zfa_data <- biolayout_zfa_data[ !(biolayout_zfa_data$ID == 'ZFA:0001093'), ]
```

```{r make_html_table, include=TRUE}
...
css_file <- gsub('^', '"', file.path(rootPath, 'output', 'biolayout-clusters.css') )
css_file <- gsub('$', '"', css_file)
...
```{r normalisation_check, include=TRUE}
...
cluster11tpm <- tpm[ rownames( clusterData[ clusterData$cluster == "Cluster011", ] ), ]
...
cluster11NormCounts <- counts(dds, normalized = TRUE )[ rownames( clusterData[ clusterData$cluster == "Cluster011", ] ), ]
# center and scale. scale works on columns. 
...
cluster11SpikeNormCounts <- counts(ddsSpikeNormalised, normalized = TRUE )[ rownames( clusterData[ clusterData$cluster == "Cluster011", ] ), ]
# center and scale. scale works on columns. 
...
pfamMappingFile <- file.path( rootPath, 'pdb_pfam_mapping.txt')
...
col_nums <- sample(seq_len(nrow(geneCor)), sampleSize)
 <- rep(rownames(geneCor), nrow(geneCor)) 
randomCor <- 
  do.call(rbind,
...
#'
plotExprByStage <- function( genes, log = FALSE, scale = TRUE  ){
