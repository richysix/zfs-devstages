...
```{r new_genes}
dataFile <- file.path(rootPath, 'dataFiles', 'rnaseq', 'zfs-rnaseq-grcz10.90.tsv')
if( !file.exists(dataFile) ){
  stop("RNA-seq counts data file is not present! Please download from figshare. See README.")
}
...
new_genes_counts <- counts(dds_90, normalized = TRUE)[ new_genes[[1]], ]

new_genes_info <- cluster_and_plot_tpms(log10(new_genes_counts + 1))

pdf(file = file.path(rootPath, 'plots', 'e90.new_genes.log10.heatmap.pdf'))
new_genes_info$plot + 
  scale_fill_gradientn( colours = c("blue", "yellow", "red"),
                        guide = guide_colorbar(title = expression(paste(log[10], "(normalised counts)", sep = '') ) ) ) +
  theme(legend.position="right")
dev.off()
