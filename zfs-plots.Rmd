...
counts_by_transcripts_file <- file.path(rootPath, 'dataFiles', 'rnaseq', 'zfs-rnaseq-grcz10.det-transcript-counts.tsv')
counts_by_transcripts <- read.table(counts_by_transcripts_file, sep = "\t", header = TRUE)

counts_by_transcripts_rab41 <- counts_by_transcripts[ counts_by_transcripts$gene_id == 'ENSDARG00000029885', ]

counts_by_transcripts_rab41.m <- melt(counts_by_transcripts_rab41,
                                      id.vars = c('transcript_id', 'gene_id'), 
                                      variable.name = 'sample', value.name = 'norm_counts')
counts_by_transcripts_rab41_list <- split(counts_by_transcripts_rab41.m, counts_by_transcripts_rab41.m$sample)
counts_by_transcripts_rab41.m <- do.call(rbind,
                                   lapply(names(counts_by_transcripts_rab41_list),
                                     function(sample, rnaseqSampleInfo){
                                       counts <- counts_by_transcripts_rab41_list[[sample]]
                                       numRows <- nrow(counts)
                                       sampleL <- rnaseqSampleInfo$sample == sample
                                       data.frame(
                                         counts,
...
counts <- counts_by_transcripts_rab41[, 3:92]
rownames(counts) <- as.character(counts_by_transcripts_rab41[, 'transcript_id'])
...
# plot for rab41 (ENSDARG00000029885)
line_colours <- colour_blind_palette[c('blue', 'vermillion', 'blue_green')]
names(line_colours) <- unique(counts_by_transcripts_rab41.m$transcript_id)

rab41_diff_isoform_plot <- ggplot(data = counts_by_transcripts_rab41.m) + 
  geom_point( aes(x = stageName, y = norm_counts, colour = transcript_id) ) + 
  geom_line(data = counts_by_transcript_by_stage.m, 
            aes(x = stageName, y = mean_counts, colour = transcript_id, group = transcript_id) ) +
  scale_colour_manual(values = line_colours, guide = guide_legend(title = 'Transcript')) +
  labs(x = 'Stage', y = 'Normalised Counts') +
  theme_minimal() + theme( axis.text.x = element_blank(),
