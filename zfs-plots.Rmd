...
for( dir in c('plots', 'topgo', file.path('topgo', 'maxByStage') ) ){
...
# PCA
# regionCount <- nrow(assay(ddsRlog))
# rv <- rowVars(assay(ddsRlog))
# select <- order(rv, decreasing=TRUE)[seq_len(min(regionCount, length(rv)))]
pca <- pcaObj$pca
# propVarPC <- pca$sdev^2 / sum( pca$sdev^2 )
# aload <- abs(pca$rotation)
...
# choose plot colours
numLevels <- nlevels(colData(ddsRlog)$stageName)
ord1 <- seq(1,numLevels,2)
ord2 <- seq(2,numLevels,2)
colourPalette <- hue_pal()(numLevels)[ order(c(ord1,ord2)) ]
...
rgl.viewpoint( theta = 30, phi = 5 )

rgl.postscript( filename =file.path(rootPath, 'plots', 'Figure.1g.pdf'), 
                fmt = 'pdf', drawText = TRUE 
)
```
...
```{r tpm_dist}
#' meanByStage
#' Calculates the mean expression for each gene for a given stage
#' 
#' This function takes a stage name and the count data and 
#'    calculates the mean expression for each gene at that stage
#'    
#' @param stageName character, the name of the stage to calculate for
#' @param countData matrix the count data to calculate on
#'
#' @return data.frame with columns stage and meanExpr
...
# calculate mean TPM for each gene for each stage
...
pdf( file.path(rootPath, 'plots', 'Figure.1-1.pdf'), width = 9, height = 6 )
print(genesDistPlot)
dev.off()
...
#'    to a particular prinicpal component. It expects the list of genes contributing 
...
#' @param ... other parameters passed to exprHeatmap
#'
#' @return tpm A numeric matrix normalized by library size and feature length
#'
...
matrixFile <- file.path(rootPath, 'plots', 'Figure.1-2.pdf' )
...
#' @param legendPosition character, one of 'left', 'right', 'top' or 'bottom'
#'
...
plotList <- add_to_plot_list( plotList, 
                              tpmMSHeatmap, 
                              file.path(rootPath, 'plots', 'Figure.3a-tpm-heatmap.pdf') )
