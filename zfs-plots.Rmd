...
```{r genes_detected_downsample}
downsampled_counts_file <- file.path(rootPath, 'dataFiles', 'rnaseq', 'zfs-rnaseq-grcz10-85-downsample.tsv')
downsampled_count_data <- read.delim(downsampled_counts_file)

# order by stage
downsampled_counts <- as.matrix(downsampled_count_data[, as.character(rnaseqSampleInfo$sample)])
rownames(downsampled_counts) <- downsampled_count_data[, 1]

# convert to tpm and filter
downsampled_tpm <- counts_to_tpm(downsampled_counts, TxLengths, insertSizes)

# divide data up by Gene and then Stage
downsampled_tpmByGeneList <- split.data.frame(downsampled_tpm, 
                                  factor(rownames(downsampled_tpm), 
                                          levels = rownames(downsampled_tpm))
                                  )
downsampled_tpmByGeneByStageList <- lapply(downsampled_tpmByGeneList,
                               function(x){ 
                                 split.data.frame(t(x), rnaseqSampleInfo$stageName) } )

# filter to genes with non-zero TPM in all replicates at at least one stage
tpmThreshold <- 0
downsampled_genesAboveThreshold <- sapply( downsampled_tpmByGeneByStageList,
                               function(geneByStageList){
                                 any( sapply(geneByStageList, 
                                             function(x){ all(x > tpmThreshold) } ) )
                               }
                        )
downsampled_tpmFilt <- downsampled_tpm[downsampled_genesAboveThreshold, ]

rnaseqTpmThreshold_vec <- c(0.001, 0.01, 0.1, 1)
pdf('plots/genes_detected_by_threshold-downsampled.pdf')
for( rnaseqTpmThreshold in rnaseqTpmThreshold_vec ){
  downsampled_rnaseqGenesDetectedByStage <- apply(downsampled_tpmFilt, 2, function(col){ sum(col >= rnaseqTpmThreshold) } )
  downsampled_rnaseqGenesDetected.m <- melt(downsampled_rnaseqGenesDetectedByStage)
  downsampled_rnaseqGenesDetected <- data.frame(
    sample = rownames(downsampled_rnaseqGenesDetected.m),
    genes = downsampled_rnaseqGenesDetected.m$value
  )
  downsampled_rnaseqGDData <- merge(downsampled_rnaseqGenesDetected, rnaseqSampleInfo)
  
  numGenesPlotRNAseq <- ggplot(data = downsampled_rnaseqGDData, aes(x = stageName, y = genes) ) +
    geom_boxplot(outlier.shape=NA, size = 0.2 ) +
    labs(x="Stage", y=paste0("Genes Detected (>= ", rnaseqTpmThreshold, " TPM)") ) +
    theme_minimal() +
    theme( axis.text.x = element_text(colour = "black", angle = 90, vjust = 0.4, hjust = 1) )
...
```{r tpm_dist_by_sample}
xLowLim <- floor(log2(min(tpmFilt[ tpmFilt > 0 ])))
xHighLim <- ceiling(log2(max(tpmFilt)))
tpm_dist_list <- lapply(rnaseqSampleInfo$sampleName,
                        function(sample){
                          dist_data <- as.data.frame(tpmFilt[ , colnames(tpmFilt) == sample ])
                          colnames(dist_data) <- 'tpm'
                          ggplot(data=dist_data, 
                                 aes(x = tpm) ) +
                            geom_histogram( breaks = seq(xLowLim,xHighLim,1), 
                                            closed = 'right' ) + 
                            scale_x_continuous(trans = log2_trans())
                        })
pdf('plots/tpm_dist_indiv_samples.pdf')
print(tpm_dist_list)
tpmFilt.m <- melt(tpmFilt)
ggplot(data=tpmFilt.m, aes(x = value) ) +
  geom_histogram( breaks = seq(xLowLim,xHighLim,1), 
                    closed = 'right' ) + 
  geom_vline(xintercept = 1) +
  scale_x_continuous(trans = log2_trans()) +
  facet_wrap(~ Var2, nrow = 9)

ggplot(data=tpmFilt.m[ grepl('1$', tpmFilt.m$Var2), ], aes(x = value) ) +
  geom_histogram( breaks = seq(xLowLim,xHighLim,1), 
                    closed = 'right' ) + 
  geom_vline(xintercept = 1) +
  scale_x_continuous(trans = log2_trans()) +
...
rnaseqTpmThreshold <- 1
num_stages <- nlevels(rnaseqSampleInfo$stageName)
a_not_b <- integer(length = num_stages - 1)
a_and_b <- integer(length = num_stages - 1)
b_not_a <- integer(length = num_stages - 1)
for( i in seq_len(nlevels(rnaseqSampleInfo$stageName) - 1)){
  first_stage_data <- melt(tpmFilt[ , grepl(levels(rnaseqSampleInfo$stageName)[i], colnames(tpmFilt)) ])
  next_stage_data <- melt(tpmFilt[ , grepl(levels(rnaseqSampleInfo$stageName)[i + 1], colnames(tpmFilt)) ])
  
  a_genes_above_theshold <- unique(first_stage_data[ first_stage_data$value >= rnaseqTpmThreshold, 'Var1' ])
  b_genes_above_theshold <- unique(next_stage_data[ next_stage_data$value >= rnaseqTpmThreshold, 'Var1' ])
  a_not_b[i] <- length(setdiff(a_genes_above_theshold, b_genes_above_theshold))
  a_and_b[i] <- length(intersect(a_genes_above_theshold, b_genes_above_theshold))
  b_not_a[i] <- length(setdiff(b_genes_above_theshold, a_genes_above_theshold))
}

venn_numbers <- data.frame(
  stage1 = levels(rnaseqSampleInfo$stageName)[1:(num_stages - 1)],
  stage2 = levels(rnaseqSampleInfo$stageName)[2:num_stages],
  stage1_not_stage2 = a_not_b,
  stage1_and_stage2 = a_and_b,
  stage2_not_stage1 = b_not_a
)

names(venn_numbers) <- c('stage1', 'stage2', 'lost', 'common', 'gained')
venn_numbers$lost <- -venn_numbers$lost
venn_numbers.m <- melt(venn_numbers, variable.name = 'category', value.name = 'num_genes',
                       id.vars = c('stage1', 'stage2'))
# remove common category
venn_numbers.m <- venn_numbers.m[ venn_numbers.m$category != 'common', ]
venn_numbers.m$stages <- paste(venn_numbers.m$stage1, venn_numbers.m$stage2,
                               sep = '-')
venn_numbers.m$stages <- factor(venn_numbers.m$stages,
                                levels = rev(unique(venn_numbers.m$stages)))

png(file = file.path(rootPath, 'plots', 'genes_lost_vs_gained_by_stage.png'),
    width = 480, height = 240)
pdf(file = file.path(rootPath, 'plots', 'genes_lost_vs_gained_by_stage.pdf'),
    width = 8, height = 4)
ggplot(data = venn_numbers.m) + 
  geom_col(aes(x = stages, y = num_genes, fill = category)) + 
  coord_flip() + 
  scale_fill_manual(values = cb_palette[2:3])
dev.off()

write.table(venn_numbers, file = file.path(rootPath, 'output', 'genes_by_stage_venn.tsv'),
...
print_log2_as_exponent <- function(log2breaks){
...
tpmByStageList <- 
  split(t(tpmFilt), rnaseqSampleInfo$stageName)
...
# 1. The ids of the named genes assigned to this stage
# 2. The ids of the unnamed genes assigned to this stage
...
```{r undetected_GO}
all_genes <- do.call(c, lapply(tpmByStage, function(x){ colnames(x) } ) )
...
for( ontologyClass in c('BP', 'MF', 'CC') ){
  ...
  sigRes <- runTopGO( undetectedGenes, ontologyClass, all_genes, outputPrefix )
}
...
pdf(file = file.path(rootPath, 'plots', 'biolayout-cluster-profiles_meanTPM.pdf'))
for( cluster_num in seq_len(length(clusters))){
  print(ggplot(data = cluster_profile_data_list[[cluster_num]], 
          aes(x = stage, y = mean_tpm, group = 1) ) +
    geom_path( size = 0.5 ) +
...
```{bash biolayout_go_zfa}
# run R script to create GO/ZFA html files
Rscript make_go_zfa_summary.R
```

```{r chr_by_stage}
names(genesInClustersList) <- levels(rnaseqSampleInfo$stageName)

chr_freq_by_stage <- do.call(rbind,
  lapply(names(genesInClustersList),
       function(stage){
         do.call(rbind,
                  lapply(c('Characterised Genes', 'Uncharacterised Genes'), 
                    function(name, stage){
                    named_status <- ifelse(name == 'Characterised Genes', 'Named', 'Unnamed')
                    genes <- genesInClustersList[[stage]][[name]]
                    info <- AllGenesInfo[genes, ]
                    df <- as.data.frame(table(info$chromosome_name))
                    df$stage <- rep(stage, nrow(df))
                    df$named <- rep(named_status, nrow(df))
                    df <- df[ df$Freq > 1, ]
                    df[ order(df$Freq, decreasing = TRUE), ][1:10,]
                  },
                  stage
                  )
        )
...
colnames(chr_freq_by_stage) <- c('Chr', 'Freq', 'stage', 'named')
write.table(chr_freq_by_stage, file = file.path(rootPath, 'output', 'chr_freq_by_stage.txt'),
            row.names = FALSE, quote = FALSE, sep = "\t")
```
...
pfamMapping <- read.delim(file=pfamMappingFile, sep="\t", header=TRUE)
...
getPfamForGenes <- function( biomartObj, genes, pfamDesc ){
  ...
  Pfam_plusDesc <- merge(genes_Pfam_domains, pfamDesc, all.x = TRUE)
  Pfam_plusDesc <-  unique(Pfam_plusDesc[, 1:4 ])
  return(Pfam_plusDesc)
}

get_lca_for_genes <- function(genes, species, biomartObj){
  homologues_for <- getBM( 
    filters = "ensembl_gene_id",
    attributes = c('ensembl_gene_id',
      paste0(species, '_homolog_subtype')
    ),
    values = as.character( genes ),
    mart = biomartObj
  )
...
species_list <- c('hsapiens', 'mmusculus', 'rnorvegicus',
             'amexicanus', 'gmorhua', 'trubripes','olatipes',
             'gaculeatus', 'tnigroviridis')
get_largest_phylum <- function(gene_list, species_list, ensBiomart){
  lca_for_genes <- lapply(species_list, function(species){
    get_lca_for_genes(gene_list, species, ensBiomart)
  })
  lca_list <- lapply(lca_for_genes, function(x){ x[[2]] })
  phylum_list <- rep('Danio', length(lca_list[[1]]))
  teleosts <- Reduce('|', 
                  lapply(lca_list, 
                    function(x){ x == 'Neopterygii' | x == 'Clupeocephala' | 
                        x == 'Otophysi' }) )
  phylum_list[teleosts] <- 'Teleosts'
  vertebrates <- Reduce('|', 
                  lapply(lca_list, 
                    function(x){ x == 'Euteleostomi' | x == 'Vertebrata' | 
                        x == 'Chordata' | x == 'Bilateria' }) )
  phylum_list[vertebrates] <- 'Vertebrates'
  return(data.frame(gene_id = lca_for_genes[[1]][[1]],
                    phylum = phylum_list) )
}

lca_info <- lapply(genesInClustersList, 
                   function(x){ lapply(x, get_largest_phylum, species_list, ensBiomart) }
)

# convert to data frame and add pfam info
names(lca_info) <- levels(rnaseqSampleInfo$stageName)
lca_info_df <- do.call(rbind, 
  do.call(rbind,
    lapply(names(lca_info),
       function(stage){
         lapply(c('Characterised Genes', 'Uncharacterised Genes'), function(name, stage){
           df <- lca_info[[stage]][[name]]
           named_status <- ifelse(name == 'Characterised Genes', 'Named', 'Unnamed')
           data.frame(
             gene_id = df$gene_id,
             stage = rep(stage, nrow(df)),
             named = rep(named_status, nrow(df)),
             phylum = df$phylum
           )
         }, stage )
       })
...
genes_by_stage_by_named_by_phylum <- 
  ddply(lca_info_df, .(stage, named, phylum), summarise, count = length(gene_id))

genes_by_stage_by_named_list <- split(genes_by_stage_by_named_by_phylum,
      interaction(genes_by_stage_by_named_by_phylum$stage,
                  genes_by_stage_by_named_by_phylum$named) )

genes_by_stage_by_named_by_phylum <- 
  do.call(rbind,
        lapply(genes_by_stage_by_named_list,
          function(x){
            x$pc_count <- x$count / sum(x$count)
            return(x)
          }
        )
)

NamedStackedPC <- ggplot( data = genes_by_stage_by_named_by_phylum[ genes_by_stage_by_named_by_phylum$named == "Named", ] ) + 
  geom_col( aes( x = stage, y = pc_count, fill = phylum ) ) + 
...
unnamedSubset <- genes_by_stage_by_named_by_phylum[ genes_by_stage_by_named_by_phylum$named == "Unnamed", ]
...
unnamedSubset$label = as.character( round( unnamedSubset$pc_count * 100, digits = 1 ) )
...
# Look at Pfam domains
pfam_for_all_genes <- getPfamForGenes(ensBiomart, lca_info_df$gene_id, pfamDesc )

genes_lca_pfam <- merge(lca_info_df, pfam_for_all_genes, 
                        by.x = 'gene_id', by.y = 'ensembl_gene_id', all.x = TRUE)

genes_lca_pfam_counts <- ddply(genes_lca_pfam, .(stage, named, phylum, pfam, pfamDescription),
                               summarise, count = length(pfam) )

# lca_pfam_data: subset of the data by stage, named and phylum
other_threshold <- 10
count_up_genes <- function(lca_pfam_data, other_threshold){
  lca_pfam_data_counts <- ddply(lca_pfam_data, .(stage, named, phylum, pfam, pfamDescription),
                               summarise, count = length(pfam) )
  # remove NA
  lca_pfam_counts_no_na <- lca_pfam_data_counts[ !is.na(lca_pfam_data_counts$pfam), ]
  
  # split into Znf, above and belowthreshold
  zinc_finger_domains <- c('PF00096', 'PF12874', 'PF13465', 'PF13894', 'PF13912')
  znf_genes <- lca_pfam_data$gene_id[ Reduce('|', lapply(zinc_finger_domains, function(x){lca_pfam_data$pfam == x}) ) ]
  znf_count <- length(unique(znf_genes[!is.na(znf_genes)]))
  if( znf_count >= other_threshold ){
    # remove from counts
    lca_pfam_counts_no_na <- lca_pfam_counts_no_na[ !Reduce('|', lapply(zinc_finger_domains, function(x){lca_pfam_counts_no_na$pfam == x}) ), ]
    # create new line
    znf_counts <- data.frame(
                    stage = lca_pfam_counts_no_na$stage[1],
                    named = lca_pfam_counts_no_na$named[1],
                    phylum = lca_pfam_counts_no_na$phylum[1],
                    pfam = 'ZnF',
                    pfamDescription = 'Zinc Finger',
                    count = znf_count
    )
  } else{
    znf_counts <- data.frame()
  ...
  above_threshold_pfam <- lca_pfam_counts_no_na$count >= other_threshold
  # count up unique genes for other
  other_pfam <- lca_pfam_counts_no_na$pfam[!above_threshold_pfam]
  other_gene_ids <- lca_pfam_data$gene_id[ Reduce('|', lapply(other_pfam, function(x){ lca_pfam_data$pfam == x })) ]
  other_counts <- 
    data.frame(
      stage = lca_pfam_counts_no_na$stage[1],
      named = lca_pfam_counts_no_na$named[1],
      phylum = lca_pfam_counts_no_na$phylum[1],
      pfam = 'Other',
      pfamDescription = 'Other',
      count = length(unique(other_gene_ids[!is.na(other_gene_ids)]))
    )
  
  pfam_counts <- rbind(
    lca_pfam_counts_no_na[ above_threshold_pfam, ],
    znf_counts,
    other_counts,
    lca_pfam_data_counts[ is.na(lca_pfam_data_counts$pfam), ]
  )
  pfam_counts$percent <- pfam_counts$count / sum(pfam_counts$count)
  
  return(pfam_counts)
}

lca_pfam_by_stage_named_phylum <- split(genes_lca_pfam, interaction(genes_lca_pfam$stage, genes_lca_pfam$named, genes_lca_pfam$phylum))
lca_pfam_unique_genes <- do.call(rbind, lapply(lca_pfam_by_stage_named_phylum, count_up_genes, other_threshold) )

# count up unnamed genes by stage
lca_pfam_by_stage <- split(lca_pfam_unique_genes, lca_pfam_unique_genes$stage)
frac_znf_by_stage <- sapply(lca_pfam_by_stage,
       function(x){
         znf <- sum( x$count[x$named == 'Unnamed' & x$pfam == 'ZnF' & !is.na(x$pfam)])
         all <- sum( x$count[x$named == 'Unnamed'])
         znf/all
       }
)

# set colours
numLevels <- nlevels(lca_pfam_unique_genes$pfam)
ord1 <- seq(1,numLevels,2)
ord2 <- seq(2,numLevels,2)
stacked_bar_palette <- hue_pal()(numLevels)[ order(c(ord1,ord2)) ]
stacked_bar_palette[1:2] <- cb_palette[c(2,1)]

stacked_bar_pfam_phylum_percent <- ggplot(data = lca_pfam_unique_genes) + 
  geom_col( aes(x = stage, y = percent, fill = pfam) ) + 
  scale_fill_manual(values = stacked_bar_palette, na.value = 'grey70') + 
  coord_flip() + facet_wrap(~ named + phylum)

pdf(file = file.path(rootPath, 'plots', 'stacked_bar_unique_named_phylum.pdf'), 
    width = 11, height = 8)
print(stacked_bar_pfam_phylum_percent + theme(legend.position = 'none'))
dev.off()

# add together anything under 10 into an other category that isn't NA
other_threshold <- 10
genes_lca_pfam_counts_other <- ddply(genes_lca_pfam_counts[ genes_lca_pfam_counts$count < other_threshold & !is.na(genes_lca_pfam_counts$pfam), ], 
                                     .(stage, named, phylum), summarise, count = sum(count) )
# add pfam and pfamDescription columns
genes_lca_pfam_counts_other$pfam <- rep('Other', nrow(genes_lca_pfam_counts_other))
genes_lca_pfam_counts_other$pfamDescription <- rep(NA, nrow(genes_lca_pfam_counts_other))
# reorder columns
genes_lca_pfam_counts_other <- genes_lca_pfam_counts_other[,c(1:3,5,6,4)]

# join back to domains over 10
genes_lca_pfam_counts_summary <- rbind(genes_lca_pfam_counts[ genes_lca_pfam_counts$count >= other_threshold | 
                                                                is.na(genes_lca_pfam_counts$pfam), ],
                                       genes_lca_pfam_counts_other)

stacked_bar_pfam <- ggplot(data = genes_lca_pfam_counts_summary) + 
  geom_col( aes(x = stage, y = count, fill = pfam) ) + 
  coord_flip() + facet_wrap(~ named) + 
  theme(legend.position = "none") 

stacked_bar_pfam_phylum <- ggplot(data = genes_lca_pfam_counts_summary) + 
  geom_col( aes(x = stage, y = count, fill = pfam) ) + 
  coord_flip() + facet_wrap(~ named + phylum) + 
  theme(legend.position = "none") 

# convert to percentage
# split by named and phylum and calculate percentages
genes_lca_pfam_counts_summary_by_named_by_phylum_by_stage <- 
  split(genes_lca_pfam_counts_summary, interaction(genes_lca_pfam_counts_summary$named, genes_lca_pfam_counts_summary$phylum, genes_lca_pfam_counts_summary$stage))

genes_lca_pfam_counts_summary <- 
  do.call(rbind,
    lapply(genes_lca_pfam_counts_summary_by_named_by_phylum_by_stage, 
           function(df){
...
stacked_bar_pfam_phylum_percent <- ggplot(data = genes_lca_pfam_counts_summary) + 
  geom_col( aes(x = stage, y = percent, fill = pfam) ) + 
  coord_flip() + facet_wrap(~ named + phylum) + 
  theme(legend.position = "none") 

# print to file
pdf(file = file.path(rootPath, 'plots', 'stacked_bar_pfam.pdf'))
print(stacked_bar_pfam)
print(stacked_bar_pfam_phylum)
print(stacked_bar_pfam_phylum_percent)
dev.off()

# aggregate phylum data
genes_lca_pfam_totals <- ddply(genes_lca_pfam, .(stage, named, pfam, pfamDescription),
                               summarise, count = length(pfam) )

# add together anything under 10 into an other category that isn't NA
other_threshold <- 10
genes_lca_pfam_totals_other <- ddply(genes_lca_pfam_totals[ genes_lca_pfam_totals$count < other_threshold & !is.na(genes_lca_pfam_totals$pfam), ], 
                                     .(stage, named), summarise, count = sum(count) )
# add pfam and pfamDescription columns
genes_lca_pfam_totals_other$pfam <- rep('Other', nrow(genes_lca_pfam_totals_other))
genes_lca_pfam_totals_other$pfamDescription <- rep(NA, nrow(genes_lca_pfam_totals_other))
# reorder columns
genes_lca_pfam_totals_other <- genes_lca_pfam_totals_other[,c('stage', 'named', 'pfam', 'pfamDescription', 'count')]

# join back to domains over 10
genes_lca_pfam_totals_summary <- rbind(genes_lca_pfam_totals[ genes_lca_pfam_totals$count >= other_threshold | 
                                                                is.na(genes_lca_pfam_totals$pfam), ],
                                       genes_lca_pfam_totals_other)

genes_lca_pfam_totals_summary_by_named_by_stage <- 
  split(genes_lca_pfam_totals_summary, interaction(genes_lca_pfam_totals_summary$named, genes_lca_pfam_totals_summary$stage))

genes_lca_pfam_totals_summary_by_named_by_stage <- 
  do.call(rbind,
    lapply(genes_lca_pfam_totals_summary_by_named_by_stage, 
           function(df){
...
ggplot(data = genes_lca_pfam_totals_summary_by_named_by_stage[ genes_lca_pfam_totals_summary_by_named_by_stage$named == 'Unnamed', ]) + 
  geom_col( aes(x = stage, y = percent, fill = pfam) ) + 
  coord_flip() + 
  theme(legend.position = "none") 

# remove Other category
# genes_lca_pfam_counts_summary_no_other <- 
#   genes_lca_pfam_counts_summary[ genes_lca_pfam_counts_summary$pfam != 'Other' &
#                                  !is.na(genes_lca_pfam_counts_summary$pfam), ]
# 
# stacked_bar_pfam <- ggplot(data = genes_lca_pfam_counts_summary_no_other) + 
#   geom_col( aes(x = stage, y = percent, fill = pfam) ) + 
#   coord_flip() + facet_wrap(~ named) + 
#   theme(legend.position = "none")
# pdf(file.path(rootPath, 'plots', 'stacked_bar_pfam.pdf'))
# print(stacked_bar_pfam)
# dev.off()


# check genes that give rise to Znf at 128-cell to 75% epi
znf_domains <- c('PF00096', 'PF13465', 'PF13894', 'PF13912')
unnamed_znf_genes <- do.call(rbind,
                             lapply(znf_domains, 
                                    function(domain){
                                      genes_lca_pfam[ genes_lca_pfam$pfam == domain & 
                                                        genes_lca_pfam$named == 'Unnamed' &
                                                        !is.na(genes_lca_pfam$pfam), ]
                                      }
                                    )
)
unnamed_znf_gene_info <- AllGenesInfo[ as.character(unique(unnamed_znf_genes$gene_id)), ]
chr_counts <- table(unnamed_znf_gene_info$chromosome_name)
chr_counts <- chr_counts[ chr_counts != 0 & !grepl("^KN", names(chr_counts)) ]
chr_counts <- melt(chr_counts)
chr_counts$Var1 <- factor(chr_counts$Var1,
                          levels = chr_counts$Var1[ order(chr_counts$Var1) ])
# plot chr counts
unnamed_znf_chr_dist <- ggplot(data = chr_counts) + geom_col( aes(x = Var1, y = value))
pdf(file.path(rootPath, 'plots', 'unnamed_znf_chr_dist.pdf'))
print(unnamed_znf_chr_dist)
dev.off()


genes_lca_pfam_counts_by_stage_by_named <- split(genes_lca_pfam_counts,                                            interaction(genes_lca_pfam_counts$stage,genes_lca_pfam_counts$named) )

domains_over_ten <- do.call(rbind,
  lapply(genes_lca_pfam_counts_by_stage_by_named,
       function(x){
         x <- x[ x$count > 10, ]
...
write.table(domains_over_ten, file = file.path(rootPath, 'output', 'pfam_domains_by_stage_by_named.tsv'),
            sep = "\t", quote = FALSE, row.names = FALSE, col.names = TRUE )
```

```{r protein_domains_unchar}
# get protein domains for named/unnamed genes at different stages
pfam_by_stage_by_named_status <- lapply(genesInClustersList, function(genes_by_named_status) {
  lapply(genes_by_named_status, 
         function(genes, ensBiomart, pfamDesc) { getPfamForGenes(ensBiomart, genes, pfamDesc) },
    ensBiomart,
    pfamDesc )
})

# find pfam domains in unnamed genes sets
pfam_counts_list <- 
  lapply(pfam_by_stage_by_named_status,
         function(pfam_for_stage){
           pfam_counts <- as.data.frame(table(pfam_for_stage[[2]]$pfam))
           names(pfam_counts) <- c('pfam', 'count')
           pfam_counts <- merge(pfam_counts, pfamDesc)
           pfam_counts[order(pfam_counts$count, decreasing = TRUE), ]
         }
        )
names(pfam_counts_list) <- levels(rnaseqSampleInfo$stageName)

lapply(pfam_counts_list,
        function(x){
           print(x[x$count > 5, ])
        }
)
...
```{r chr4_genes_plot, include=TRUE}
chr4_GeneExprList <- plotExprByChr( '4', tpm = tpmFilt )
```

```{r chr4Expr_plus_cluster_col, include=TRUE}
...
for( i in seq_len(length(corThresholds))  ){
  ...
  for( ontologyClass in c('BP', 'MF', 'CC') ){
    ...
    sigRes <- runTopGO( cor_genes, ontologyClass, all_genes, outputPrefix, sigLevel = 0.05 )
  }
...
# Remove genes with no pfam
high_cor_genes_Pfam <- high_cor_genes_Pfam[ high_cor_genes_Pfam$pfam != '', ]

# count up numbers of genes with each pfam domain
all_genes_pfam <- getPfamForGenes( ensBiomart, all_genes, pfamDesc )
# Remove genes with no pfam
all_genes_pfam <- all_genes_pfam[ all_genes_pfam$pfam != '', ]
...
postscript(file.path(rootPath, 'plots', 'Figure.9-1.spearman.eps'))
print(detctSampleCor_spearman_heatmap)
dev.off()
...
```{bash organise_detct_dir}
mkdir output/pca/detct
mv output/pca/PC* output/pca/detct/
# edit files to replace gene_id in header with region
...
output/topgo/mult-ends/1551.gene-list.tsv > output/topgo/mult-ends/stricter-mult-ends.$ontology.enriched-GO.tsv
done
...
# Remove genes with no pfam
mult_ends_genes_Pfam <- mult_ends_genes_Pfam[ mult_ends_genes_Pfam$pfam != '', ]
mult_ends_genes_pfam_counts <- table(mult_ends_genes_Pfam$pfam)

# count up numbers of genes with each pfam domain
all_genes_pfam <- getPfamForGenes( ensBiomart, as.character(AllGenesInfo$ensembl_gene_id), pfamDesc )
all_genes_pfam <- all_genes_pfam[ all_genes_pfam$pfam != '', ]
pfam_counts <- table(all_genes_pfam$pfam)
...
```{r new_genes}
dataFile <- file.path(rootPath, 'dataFiles', 'rnaseq', 'zfs-rnaseq-grcz10_e90_tpm_genes.tsv')
tpm_e90 <- read.table(dataFile, header = TRUE, sep ="\t", quote = "\"")
rownames(tpm_e90) <- as.character(tpm_e90[[1]])

tpm_e90_mat <- as.matrix(tpm_e90[,9:ncol(tpm_e90)])
rownames(tpm_e90_mat) <- as.character(tpm_e90[[1]])
colnames(tpm_e90_mat) <- as.character(rnaseqSampleInfo$sampleName)

# divide data up by Gene and then Stage
tpm_e90_ByGeneList <- split.data.frame(tpm_e90_mat, 
                                  factor(rownames(tpm_e90_mat), 
                                          levels = rownames(tpm_e90_mat))
                                  )
tpm_e90_ByGeneByStageList <- lapply(tpm_e90_ByGeneList,
                               function(x){ 
                                 split.data.frame(t(x), rnaseqSampleInfo$stageName) } )

# filter to genes with non-zero TPM in all replicates at at least one stage
tpmThreshold <- 0
genesAboveThreshold <- sapply( tpm_e90_ByGeneByStageList,
                               function(geneByStageList){
                                 any( sapply(geneByStageList, 
                                             function(x){ all(x > tpmThreshold) } ) )
                               }
                        )
tpm_e90_Filt <- tpm_e90_mat[genesAboveThreshold, ]
...
new_genes_annotation <- tpm_e90[as.character(new_genes[[1]]), c('Gene.ID', 'Chr', 'Start', 'End', 'Biotype', 'Name', 'Description')]

new_genes_unnamed <- Reduce('|',
                            lapply( as.list(patterns),
                              function( pattern, new_genes_annotation ){ grepl(pattern, new_genes_annotation$Name, perl = TRUE ) },
                              new_genes_annotation )
)
# number of unnamed genes
sum(new_genes_unnamed)
# 


# divide data up by Gene and then Stage
filter_tpm <- function(tpm, tpmThreshold = 0){
  tpm_ByGeneList <- split.data.frame(tpm, 
                                  factor(rownames(tpm), 
                                          levels = rownames(tpm))
                                  )
  tpm_ByGeneByStageList <- lapply(tpm_ByGeneList,
                                   function(x){ 
                                     split.data.frame(t(x), rnaseqSampleInfo$stageName) } )

  # filter to genes with non-zero TPM in all replicates at at least one stage
  genesAboveThreshold <- sapply( tpm_ByGeneByStageList,
                                 function(geneByStageList){
                                   any( sapply(geneByStageList, 
                                               function(x){ all(x > tpmThreshold) } ) )
                                 }
                        )
  tpm_filt <- tpm[genesAboveThreshold, ]
}

new_genes_Filt <- filter_tpm(new_genes_tpm)
new_genes_info <- cluster_and_plot_tpms(log10(new_genes_Filt + 1))
...
# 
mean_expr_protein_coding <- apply(tpm_e90_mat[tpm_e90$Biotype == 'protein_coding', ], 1, mean)
new_genes_L <- Reduce('|', lapply(rownames(new_genes_tpm), 
                                function(x){ names(mean_expr_protein_coding) == x }) )
mean_expr_new_genes <- mean_expr_protein_coding[new_genes_L]
mean_expr_protein_coding <- mean_expr_protein_coding[!new_genes_L]

mean_expr_new_genes.m <- melt(mean_expr_new_genes)
mean_expr_new_genes.m$type <- rep('new', nrow(mean_expr_new_genes.m))
mean_expr_protein_coding.m <- melt(mean_expr_protein_coding)
mean_expr_protein_coding.m$type <- rep('existing', nrow(mean_expr_protein_coding.m))

mean_expr_data <- rbind(mean_expr_new_genes.m, mean_expr_protein_coding.m)
mean_expr_data$type <- factor(mean_expr_data$type,
                              levels = c('existing', 'new'))

protein_coding_e89_median_mean_tpm <- median(mean_expr_data$value[ mean_expr_data$type == 'existing'])
protein_coding_e90_median_mean_tpm <- median(mean_expr_data$value[ mean_expr_data$type == 'new'])

# plot distributions
mean_expr_dist <- ggplot(data=mean_expr_data) + 
  geom_vline(xintercept = c(protein_coding_e89_median_mean_tpm, protein_coding_e90_median_mean_tpm),
             colour = c('firebrick2', 'steelblue3'),
             linetype = c(5,2)) +
  geom_density(aes(x = value, fill = type), alpha = 0.4) + 
  scale_x_continuous(limits = c(0.0078125,1024), trans = log2_trans(),
                     labels = print_log2_as_exponent) +
  labs(x = expression(paste('Mean Expression ', log[2], '(TPM)')), y = 'Frequency') +
  scale_fill_manual(values = c('firebrick2', 'steelblue3'), 
                    labels = c('Ensembl v89', 'Ensembl v90')) +
  guides(fill = guide_legend(title = 'Type', override.aes = list(alpha = 1))) +
  theme_minimal()

# postscript doesn't support semi-transparency
pdf(file = file.path(rootPath, 'plots', 'fig7.mean_expr_dist.pdf'),
           height = 7, width = 7)
print(mean_expr_dist + theme(axis.text = element_text(size = 12),
...
pdf(file = file.path(rootPath, 'plots', 'e90.new_lincRNAs.filtered.log10.tpm.heatmap.sample_strip.pdf'))
grid.newpage()
grid.draw( 
  arrangeGrob( grobs = list( cluster_and_plot_tpms(log10(new_lincs_Filt + 1))$plot + 
                               scale_fill_gradientn( colours = c("blue", "yellow", "red"),
                                    guide = guide_colorbar(title = expression(paste(log[10], "(TPM)", sep = '') ) ) ) +
                               theme(legend.position="right"),
                             sampleStrip + theme( plot.margin = unit(c(0,0.152,0,0), unit = 'npc') ) ),
...
# expression distribution (mean TPM)
new_lincs_mean_tpm <- apply(new_lincs_tpm, 1, mean)
new_lincs_mean_tpm.m <- melt(new_lincs_mean_tpm)
new_lincs_mean_tpm.m$biotype <- rep('lincRNA', nrow(new_lincs_mean_tpm.m))

# subset tpm to protein coding
coding_genes_file <- file.path(rootPath, 'dataFiles', 'e90_protein_coding_ids.txt')
coding_genes <- read.table(file = coding_genes_file, header = FALSE)
coding_genes_e90_tpm <- tpm_e90_mat[ as.character(coding_genes[, 1]), ]
coding_genes_mean_tpm <- apply(coding_genes_e90_tpm, 1, mean)
coding_genes_mean_tpm.m <- melt(coding_genes_mean_tpm)
coding_genes_mean_tpm.m$biotype <- rep('protein_coding', nrow(coding_genes_mean_tpm.m))

mean_tpm_by_biotype <- rbind(coding_genes_mean_tpm.m, new_lincs_mean_tpm.m)
mean_tpm_by_biotype$biotype <- factor(mean_tpm_by_biotype$biotype,
                                 levels = c('protein_coding', 'lincRNA'))

new_lincs_median_mean_tpm <- median(mean_tpm_by_biotype$value[ mean_tpm_by_biotype$biotype == 'lincRNA'])
coding_genes_median_mean_tpm <- median(mean_tpm_by_biotype$value[ mean_tpm_by_biotype$biotype == 'protein_coding'])

new_lincs_mean_mean_tpm <- mean(mean_tpm_by_biotype$value[ mean_tpm_by_biotype$biotype == 'lincRNA'])
coding_genes_mean_mean_tpm <- mean(mean_tpm_by_biotype$value[ mean_tpm_by_biotype$biotype == 'protein_coding'])

new_linc_tpm_dist <- ggplot(data = mean_tpm_by_biotype) + 
  geom_vline(xintercept = c(coding_genes_median_mean_tpm, new_lincs_median_mean_tpm),
             colour = c('firebrick2', 'steelblue3'),
             linetype = c(5,2)) +
  geom_density(aes(x = value, fill = biotype), alpha = 0.3 ) + 
  labs(x = expression(paste('Mean Expression ', log[2], '(TPM)')), y = 'Frequency') +
  scale_x_continuous(trans = log2_trans(), limits = c(2^-8,2^13),
                     labels = print_log2_as_exponent ) +
  scale_fill_manual(values = c('firebrick2', 'steelblue3')) +
  guides(fill = guide_legend(title = 'Biotype', override.aes = list(alpha = 1))) + 
  theme_minimal()

# postscript doesn't support semi-transparency
pdf(file = file.path(rootPath, 'plots', 'e90_linc_tpm_dist.pdf'),
           height = 7, width = 7)
print(new_linc_tpm_dist + theme(axis.text = element_text(size = 12),
                             axis.title = element_text(size = 16),
                             legend.text = element_text(size = 12),
                             legend.title = element_text(size = 16)
                             ))
...
max_expr_coding <- apply(coding_genes_e90_tpm, 1, max)
max_expr_linc <- apply(new_lincs_tpm, 1, max)
max_expr_df <- data.frame(
  biotype = c(rep('protein_coding', length(max_expr_coding)),
              rep('lincRNA', length(max_expr_linc))),
  max_expr = c(max_expr_coding, max_expr_linc)
)
max_expr_df$biotype <- factor(max_expr_df$biotype,
                              levels = c('protein_coding', 'lincRNA'))

new_linc_max_tpm_dist <- ggplot(data = max_expr_df) + 
  geom_density(aes(x = max_expr, fill = biotype), alpha = 0.3 ) + 
  scale_x_continuous(trans = log2_trans(), limits = c(0.125,32768)) +
  scale_fill_manual(values = c('firebrick2', 'steelblue3')) +
  guides(fill = guide_legend(override.aes = list(alpha = 1)))

pdf(file = file.path(rootPath, 'plots', 'e90_linc_max_tpm_dist.pdf'))
print(new_linc_max_tpm_dist)
dev.off()


# limit to most highly expressed for promoter analysis
# order by max expression and take top 25%
max_expr_linc <- max_expr_linc[ order(max_expr_linc, decreasing = TRUE) ]
top25_linc <- names(max_expr_linc[1:ceiling(length(max_expr_linc)*0.25)])

write.table(top25_linc, file = file.path(rootPath, 'output', 'top_0.25_linc_genes.txt'),
            quote = FALSE, row.names = FALSE, col.names = FALSE)

# also write out filtered set
write.table(rownames(new_lincs_Filt), file = file.path(rootPath, 'output', 'linc_genes_expressed.txt'),
            quote = FALSE, row.names = FALSE, col.names = FALSE)
...
```{r protein_coding_stats}
protein_coding_exon_info_file <- file.path(rootPath, 'dataFiles', '2017-09-28_e90_protein-coding_exon-info.tsv')
protein_coding_exon_info <- read.table(protein_coding_exon_info_file, sep ="\t")
names(protein_coding_exon_info) <- c('gene_id', 'transcript_id', 'exon_id', 'chr', 'start', 
                                     'end', 'strand')

protein_coding_exon_info$exon_size = protein_coding_exon_info$end - protein_coding_exon_info$start + 1
mean(protein_coding_exon_info$exon_size)
# [1] 235.2692
median(protein_coding_exon_info$exon_size)
# [1] 131

# # plot histogram
# ggplot(data = protein_coding_exon_info) + geom_histogram( aes(x = exon_size) )

# plot histgram of exon sizes
new_genes_L <- Reduce('|', lapply(rownames(new_genes_tpm), 
                                  function(x){ protein_coding_exon_info$gene_id == x }) )
new_genes_exon_info <- protein_coding_exon_info[new_genes_L, ]
existing_genes_exon_info <- protein_coding_exon_info[!new_genes_L, ]

protein_coding_exon_info$gene_type <- rep('existing', nrow(protein_coding_exon_info))
protein_coding_exon_info$gene_type[new_genes_L] <- 'new'
protein_coding_exon_info$gene_type <- factor(protein_coding_exon_info$gene_type,
                                                   levels = c('new', 'existing'))

mean(protein_coding_exon_info$exon_size[new_genes_L])
mean(protein_coding_exon_info$exon_size[!new_genes_L])

protein_coding_exon_size_hist <- ggplot(data = protein_coding_exon_info) + 
  geom_histogram( aes(x = exon_size), binwidth = 100 ) + 
  scale_x_continuous(limits = c(0,3000)) +
  facet_wrap( ~ gene_type, scales = "free_y") +
  geom_vline(data = mean_exons, aes(xintercept = median), colour = 'red') +
  geom_text(data = labels,
            aes(x = x, y = y, label = text), nudge_x = 5) + 
  labs(x = "Number of Exons", y = "Frequency") + 
  theme_minimal() +
  theme( axis.text = element_text(size = 12),
         axis.title = element_text(size = 14) )

# aggregate info to transcript
protein_coding_transcript_info <- ddply(protein_coding_exon_info, .(gene_id, transcript_id),
                              summarise, num_exons = length(exon_id),
                              transcript_length = sum(exon_size))

# mean/median number of exons
summary(protein_coding_transcript_info)

new_genes_L <- Reduce('|', lapply(rownames(new_genes_tpm), 
                                  function(x){ protein_coding_transcript_info$gene_id == x }) )
new_genes_transcript_info <- protein_coding_transcript_info[new_genes_L, ]
existing_genes_transcript_info <- protein_coding_transcript_info[!new_genes_L, ]

protein_coding_transcript_info$gene_type <- rep('existing', nrow(protein_coding_transcript_info))
protein_coding_transcript_info$gene_type[new_genes_L] <- 'new'
protein_coding_transcript_info$gene_type <- factor(protein_coding_transcript_info$gene_type,
                                                   levels = c('new', 'existing'))

# plot histograms of num exons and transcript length
mean_exons <- mean(protein_coding_transcript_info$num_exons)
median_exons <- median(protein_coding_transcript_info$num_exons)
average_labels <- data.frame(
  x = as.numeric( c(median_exons, 10) ),
  y = c(2300, 2300),
  text = as.character(c(median_exons, sprintf('mean = %.2f', mean_exons)))
)

median_exons <- ddply(protein_coding_transcript_info, .(gene_type),
                    summarise, mean = mean(num_exons),
                    median = median(num_exons)
)

labels <- data.frame(
  gene_type = rep(median_exons$gene_type, 2),
  text = c(sprintf('median = %.0f', median_exons$median),
           sprintf('mean = %.2f', median_exons$mean)),
  x = c(median_exons$median, median_exons$mean + 30),
  y = rep(c(50, 5600), 2)
)

protein_coding_num_exons_hist <- ggplot(data = protein_coding_transcript_info) + 
  geom_histogram( aes(x = num_exons), binwidth = 1 ) + 
...
postscript(file = file.path(rootPath, 'plots', 'fig7.num_exons-protein_coding.histogram.eps'),
           width = 10, height = 7)
print(protein_coding_num_exons_hist)
dev.off()

# histogram of transcript length
mean_length <- mean(protein_coding_transcript_info$transcript_length)
median_length <- median(protein_coding_transcript_info$transcript_length)
average_labels <- data.frame(
  x = as.numeric( c(median_length, 5000) ),
  y = c(410, 410),
  text = as.character(c(median_length, sprintf('mean = %.0f nt', mean_length)))
)
protein_coding_length_hist <- ggplot(data = protein_coding_transcript_info) + 
  geom_histogram( aes(x = transcript_length), binwidth = 100 ) +
  geom_vline(xintercept = median_length, colour = 'red') +
  geom_text(data = average_labels, 
            aes(x = x, y = y, label = text), colour = c('red', 'black'), nudge_x = 400) +
  scale_x_continuous( limits = c(0,7000)) + 
  labs(x = 'Transcript Length', y = 'Frequency') + 
  theme_minimal() +
  theme( axis.text = element_text(size = 12),
         axis.title = element_text(size = 14) )

postscript(file = file.path(rootPath, 'plots', 'fig7.length.histogram.eps'),
           width = 7, height = 7)
print(protein_coding_length_hist)
dev.off()

```

```{r}
# connect to e90 biomart
e90_ensBiomart <- useMart( "ensembl", dataset="drerio_gene_ensembl")

species_list <- c('hsapiens', 'mmusculus', 'rnorvegicus',
             'amexicanus', 'gmorhua', 'trubripes','olatipes',
             'gaculeatus', 'tnigroviridis')

e90_lca_info <- get_largest_phylum(rownames(new_genes_tpm), species_list, e90_ensBiomart) 

table(e90_lca_info$phylum)

# get paralogues
danio_paralogues <- getBM( 
    filters = "ensembl_gene_id",
    attributes = c('ensembl_gene_id',
      'drerio_paralog_subtype'
    ),
    values = rownames(new_genes_tpm),
    mart = e90_ensBiomart
  )
...
```{r lincRNA_stats}
linc_exon_info_file <- file.path(rootPath, 'dataFiles', '2017-09-19_e90_lincRNA-exon-info.tsv')
linc_exon_info <- read.table(linc_exon_info_file, sep ="\t")
names(linc_exon_info) <- c('gene_id', 'transcript_id', 'exon_id', 'chr', 'start', 'end', 'strand')

# linc_exon_info
linc_exon_info$exon_size = linc_exon_info$end - linc_exon_info$start + 1
mean(linc_exon_info$exon_size)
# [1] 331.1276
median(linc_exon_info$exon_size)
# [1] 167

# plot histogram
ggplot(data = linc_exon_info) + geom_histogram( aes(x = exon_size) )

# aggregate info to transcript
linc_transcript_info <- ddply(linc_exon_info, .(gene_id, transcript_id),
                              summarise, num_exons = length(exon_id),
                              transcript_length = sum(exon_size))

# mean/median number of exons
summary(linc_transcript_info)

# plot histograms of num exons and transcript length
mean_exons <- mean(linc_transcript_info$num_exons)
median_exons <- median(linc_transcript_info$num_exons)
average_labels <- data.frame(
  x = as.numeric( c(median_exons, 10) ),
  y = c(2300, 2300),
  text = as.character(c(median_exons, sprintf('mean = %.2f', mean_exons)))
)
linc_num_exons_hist <- ggplot(data = linc_transcript_info) + 
  geom_histogram( aes(x = num_exons), binwidth = 1 ) + 
  geom_vline(xintercept = median_exons, colour = 'red') +
  geom_text(data = average_labels,
            aes(x = x, y = y, label = text), colour = c('red', 'black'), nudge_x = 0.4) + 
...
postscript(file = file.path(rootPath, 'plots', 'fig7.num_exons.histogram.eps'),
           width = 7, height = 7)
print(linc_num_exons_hist)
dev.off()

# histogram of transcript length
mean_length <- mean(linc_transcript_info$transcript_length)
median_length <- median(linc_transcript_info$transcript_length)
average_labels <- data.frame(
  x = as.numeric( c(median_length, 5000) ),
  y = c(410, 410),
  text = as.character(c(median_length, sprintf('mean = %.0f nt', mean_length)))
)
linc_length_hist <- ggplot(data = linc_transcript_info) + 
  geom_histogram( aes(x = transcript_length), binwidth = 100 ) +
  geom_vline(xintercept = median_length, colour = 'red') +
  geom_text(data = average_labels, 
            aes(x = x, y = y, label = text), colour = c('red', 'black'), nudge_x = 400) +
