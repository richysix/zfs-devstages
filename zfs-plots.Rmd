...
```{r h3k4_chip_seq}
# Get a list of genes expressed and not expressed at Dome and Prim-5 for comparing to the ChIP-Seq
...
```{r reads_mapped_rnaseq, include=TRUE}
...
There are `r sum(table(chr4ClusterData$cluster) > 8)` clusters with more than 8 genes on chr 4. Need a colour palette for 9 clusters.

```{r set_colour_palette_znf_clusters}
...
### Figure 4C - ZnF clusters expression profiles
...
### Figure 4D - histograms of ZnF/NLR genes on chr 4
...
### Figure 4E - NLR genes tpm heatmap
...
pdf(file = file.path(rootPath, 'plots', 'Figure7A.pdf'))
grid.newpage()
grid.draw( 
  arrangeGrob( grobs = list( new_genes_info$plot + 
                               scale_fill_gradientn( colours = c("blue", "yellow", "red"),
...
pdf(file = file.path(rootPath, 'plots', 'Figure7-2A.mean_expr_dist.pdf'),
           height = 7, width = 7)
...
postscript(file = file.path(rootPath, 'plots', 'Figure7B.rab41.diff-isoform.eps'),
           width = 5, height = 3.5)
...
diff_isoform_plots_file <- file.path(rootPath, 'plots', 'diff-isoform.pdf')
if (!file.exists(diff_isoform_plots_file)) {
  pdf(file = diff_isoform_plots_file)
  diff_iso_plot_list <- lapply(as.character(head(levels(tpm_by_transcripts$gene_id))),
                               function(gene){
                                 print(differential_isoform_plot(gene, tpm_by_transcripts, colour_palette))
                               }
  )
  dev.off()
}
...
postscript(file = file.path(rootPath, 'plots', 'Figure7C.eps'),
           width = 7, height = 7)
print(isoform_hist)
dev.off()
...
```{r lincRNA_heatmap, include=TRUE}
# lincRNA
...
pdf(file = file.path(rootPath, 'plots', 'Figure7B.pdf'))
grid.newpage()
grid.draw( 
...
# postscript doesn't support semi-transparency
pdf(file = file.path(rootPath, 'plots', 'Figure7-2B.e90_linc_tpm_dist.pdf'),
           height = 7, width = 7)
...
postscript(file = file.path(rootPath, 'plots', 'Figure7G.num_exons.histogram.eps'),
           width = 7, height = 7)
...
postscript(file = file.path(rootPath, 'plots', 'Figure7F.length.histogram.eps'),
           width = 7, height = 7)
